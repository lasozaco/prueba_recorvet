...
test.describe('Login en testing.recorvet.com', () => {
  ...
  test('debe iniciar sesión correctamente con credenciales válidas', async ({ page }) => {
    ...
    // Dentro del modal, seleccionar "Nota Clínica" (botón junto al heading del mismo nombre)
    await modal.getByRole('heading', { name: /nota clinica|nota clínica/i }).locator('xpath=preceding-sibling::button[1]').click();

    // El formulario de nota clínica se muestra en un modal; esperar el dialog que contiene el campo categoria
    const formModal = page.getByRole('dialog');
    await formModal.waitFor({ state: 'visible', timeout: 100000 });
    const descripcionNota = page.locator('#descripcion_nota');
    await descripcionNota.waitFor({ state: 'visible', timeout: 10000 });
    await descripcionNota.fill('otitis');

    await formModal.getByRole('button', { name: /Guardar|guardar/i }).click();
  });
});
